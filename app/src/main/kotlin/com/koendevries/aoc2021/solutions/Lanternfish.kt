...
typealias Lanternfish = List<InternalTimer>
typealias Cache<K, V> = MutableMap<K, V>

const val DEFAULT_REPRODUCTION_DAYS = 7L
const val INITIAL_REPRODUCTION_DAYS = 9L

private fun familySizeAfter(days: Long, familySizeAfterDaysCache: Cache<Long, Long>): Long =
    familySizeAfterDaysCache.getOrPut(days) {
        if (days < 1) {
            1L
        } else {
            familySizeAfter(days - DEFAULT_REPRODUCTION_DAYS, familySizeAfterDaysCache)
                .plus(familySizeAfter(days - INITIAL_REPRODUCTION_DAYS, familySizeAfterDaysCache))
        }
    }

fun Lanternfish.count(days: Int): Long {
