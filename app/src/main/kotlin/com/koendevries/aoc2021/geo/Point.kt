...
private fun countSteps(edge: Pair<Point, Point>): Int = difference(edge).run { if (x != 0) x.absoluteValue else y.absoluteValue }

private fun step(edge: Pair<Point, Point>): Point = difference(edge).run { Point(step(x), step(y)) }
